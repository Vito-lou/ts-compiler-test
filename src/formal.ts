...
function generateTsCode(intermediateJson: any): ts.SourceFile {
    ...
    // 创建函数名
    const functionName = `logicRun_${intermediateJson.chainId || 'default'}`;

    // 注入输入变量
    const parameters: ts.ParameterDeclaration[] = intermediateJson.vars
        .filter((v: any) => v.varType === 'INPUT')
        .map((v: any) => ts.factory.createParameterDeclaration(
            undefined,
            undefined,
            v.key,
            undefined,
    ...
    // 创建函数体
    const functionBody = ts.factory.createBlock(statements, true);
